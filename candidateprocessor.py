...
def CWD(dir):
    """ Helper function to join the current working directory to another path
        Used for Linux compatibility """
...
def generate_sign(name, position, district, party, image, output):
    ...
    font = ImageFont.truetype(CWD("Assets/font.ttf"), 300)
    draw = ImageDraw.Draw(sign)
    text = f"{name}\n{position}\nOf\n{district}"
    text_w, text_h = draw.multiline_textsize(text, font=font, spacing=20)
    draw.multiline_text(((2401 - text_w) / 2 + 253, 1400), text, (0, 0, 0),
...
def send_email(receiver_email, filename):
    ...
    with open(CWD(os.path.join("..", "mailcreds.txt")), "r") as file:
        password = file.read()
...
def send_error_email(receiver_email, subject, body):
    """ Sends an email, used for errors """
